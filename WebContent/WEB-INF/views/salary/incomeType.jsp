...
<head>
	...
	<script>
        ...
        var jsonStr = '{"incomeType.incomeTypeId":"_incomeTypeId","incomeType.name":"_name",' +
			'"incomeType.des":"_des",'+'"incomeType.status":"_status"}';
        $(function() {
            setPagination("list");
        });
        ...
        function showEditProductWin() {
            ...
            if (rows.length > 1) {
                ...
                var row = rows[0];
                if (row) { // 有选中的行
                    var jsonStr1 = jsonStr.replace("_incomeTypeId", row.incomeTypeId).replace("_name", row.name).
					replace("_des", row.des).replace("_des", row.des).replace("_status", row.status);
                    authority('com.cs.action.IncomeTypeAction.update', jsonStr1, "update");//+++
                }
        ...
        function disable(index){   // 禁用
    		...
    		if(row){
    			$.post("<%=path %>/incomeType/incomeTypeDisable?id=" + row.incomeTypeId,
    				$("#editForm").serialize(),
    				function(data){
    					$('#list').datagrid('reload');
    			},"json");
    	...
    	function activate(index){  // 启用
    		var row = $('#list').datagrid('getData').rows[index];
    		if(row){
    			$.post("<%=path %>/incomeType/incomeTypeActivate?id=" + row.incomeTypeId,
    				$("#editForm").serialize(),
    				function(data){
...
<table id="list" class="easyui-datagrid"
	...
	<tr>
		...
		<th data-options="field:'name',width:100">收入名称</th>
		<th data-options="field:'des',width:80">收入描述</th>
...
<div id="addWin" class="easyui-window" title="添加收入类型" data-options="closed:true" style="width:500px;height:300px;">
	<form id="addForm">
		<table>
			<tr>
				<td>收入名称</td>
				<td><input class="easyui-validatebox easyui-textbox" name="incomeType.name" data-options="required:true,validType:'length[2,20]'"/></td>
			</tr>
			<tr>
				<td>收入描述</td>
				<td><input class="easyui-validatebox easyui-textbox" name="incomeType.des" data-options="required:true,validType:'length[2,20]'"/></td>
			</tr>
			<tr>
				<td>
					<a href="javascript:;" onclick="addIncomeType();" class="easyui-linkbutton" data-options="iconCls:'icon-save'">确认</a>
				</td>
			</tr>
		</table>
	</form>
...
<div id="editWin" class="easyui-window" title="修改学生信息" data-options="closed:true" style="width:500px;height:300px;">
	<form id="editForm">
		...
		<table>
			...
			<tr>
				<td>
					<a href="javascript:;" onclick="editIncomeType();" class="easyui-linkbutton" data-options="iconCls:'icon-save'">修改</a>
				</td>
			</tr>
		</table>
	</form>
