    			...
    			function(data) {
    				if (data.result.result == "success") {
    					if (method == "save") {
    						...
    						$("#empDetailsForm").form("load", JSON.parse(str));
    						document.getElementById("empDetails").style.display="";// 显示隐藏窗口
    					}
...
<script>
	            ...
	            if (rows.length > 1) {
	                ...
	                if (row) { // 有选中的行
	                	if(row.empId){// 假如员工id有, 则拿选中行中所有数据
	                		...
	                		authority('com.cs.action.EmpAction.update', jsonStr1, "update"); 
	                   	  /* document.getElementById("empDetails").style.display=""; */
	              		  }else{
	              			 authority('com.cs.action.EmpAction.save', "", "save");
		                	}
		                }
	...
	function getRoom1(){
		var rows = $("#roomTable1").datagrid("getSelections");// 获取表格中选中行的数据
		 ...
		 $("#room1").textbox('setValue',rows[0].depId);
	...
	function getRole1(){
		var rows = $("#roleTable1").datagrid("getSelections");// 获取表格中选中行的数据
		 ...
		 $("#role1").textbox('setValue',rows[0].roleId);
...
<table id="list" class="easyui-datagrid"
	...
	<tr>
		...
		<th data-options="field:'hireDay',width:125"  formatter="formatTime">入职时间</th>
		<th data-options="field:'resignDay',width:125"  formatter="formatTime">离职时间</th>
...
<!-- 组合搜索框div -->
	...
	<div data-options="name:'e.name'">名称</div>
	    <div data-options="name:'r.name'">职位</div>
	    <div data-options="name:'d.name'">部门</div>
	    <div data-options="name:'e.idcard'">身份证号</div>
	    <div data-options="name:'e.phone'">手机号</div>
	</div>
	...
	<input type="hidden" name="emp.empId"/>
		<table id="empDetailsTable">
			<tr>
				...
				<td>
					<a href="javascript:;" onclick="showRoomAll1();">
						<input id="roomInput1" style="width:163px;" class="easyui-textbox easyui-validatebox" name="emp.deptName" data-options="required:true"/>
						<input type="hidden" id="room1" class="easyui-textbox" name="depId"/>
					</a>
				</td>
				<td>&nbsp;&nbsp;职位</td>
				<td>
					<a href="javascript:;" onclick="showRoleAll1();">
						<input id="roleInput1"  style="width:163px;" class="easyui-textbox easyui-validatebox" name="emp.roleName" data-options="required:true"/>
						<input type="hidden" id="role1" class="easyui-textbox" name="roleId"/>
					</a>
				</td>
				<td><input type="hidden" class="easyui-validatebox easyui-textbox " name="emp.pwd"/></td>
			</tr>
			<tr>
				<td>&nbsp;&nbsp;身份证号</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.idCard" data-options="required:true,validType:'IDCard'"/></td>
				<td>&nbsp;&nbsp;籍贯</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.nation" data-options="required:true"/></td>
				<td>&nbsp;&nbsp;性别</td>
				<td>
					<select style="width:175px;" class="easyui-combobox" name="emp.gender" data-options="editable:false"><option value="boy">男</option><option value="girl">女</option></select>
				</td>
				<td>&nbsp;&nbsp;指纹编号</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.fingerNo"/></td>
			</tr>
			<tr>
				<td>&nbsp;&nbsp;生日</td>
				<td><input class="easyui-datetimebox" name="emp.birthDay" data-options="editable:false"></td>
				<td>&nbsp;&nbsp;邮箱</td>
				<td><input class="easyui-validatebox textbox" style="height:22px;" name="emp.email"  data-options="required:true,validType:'email'"></td>
				<td>&nbsp;&nbsp;手机号码</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.phone" data-options="required:true,validType:'Mobile'"/></td>
				<td>&nbsp;&nbsp;QQ</td>
			...
			<tr>
				...
				<td>&nbsp;&nbsp;是否结婚</td>
				<td>
					<select style="width:175px;" class="easyui-combobox" name="emp.married" data-options="editable:false"><option value="yes">是</option><option value="no">否</option></select>
				</td>
				<td>联系人姓名</td>
			...
			<tr>
				<td>联系人手机号码</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.contactPhone" data-options="required:true,validType:'Mobile'"/></td>
				<td>&nbsp;&nbsp;毕业学校</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.college" data-options="required:true"/></td>
				<td>&nbsp;&nbsp;专业</td>
				<td>
					<select class="easyui-combobox" name="emp.major" style="width:175px;" data-options="editable:false">
				...
				<td>&nbsp;&nbsp;学历</td>
				<td>
					<select class="easyui-combobox" name="emp.eduback" style="width:175px;" data-options="editable:false">
			...
			<tr>
				<td>&nbsp;开户行名称</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.bankName"/></td>
				<td>&nbsp;银行卡姓名</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.accountName"/></td>
				<td>&nbsp;银行卡账号</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.accountNo" data-options="validType:'length[1,19]'"/></td>
				<td>&nbsp;支付宝账号</td>
				<td><input class="easyui-validatebox easyui-textbox" name="emp.alipay" data-options="validType:'length[1,11]'" /></td>
			</tr>
			<tr>
				<td>&nbsp;&nbsp;入职时间</td>
