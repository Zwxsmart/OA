	   ...
	   var jsonStr = '{"stu.stuId":"_stuId","stu.stuNo":"_stuNo",' +
        ...
        function saveProduct() {
            if ($("#saveForm").form("validate")) {
                $.post("<%=path %>/intention/saveByIndexId",
                    $("#saveForm").serialize(),
                    function (data) {
        ...
        function editProduct() {
            if ($("#editForm").form("validate")) {
                $.post("<%=path %>/intention/updateByIndexId",
                    $("#editForm").serialize(),
                    function (data) {
                        if (data.result.result == "success") {
...
<table id="list" class="easyui-datagrid"
	   data-options="toolbar:'#tb',
			singleSelect:false,
			collapsible:true,
			url:'<%=path%>/intention/queryAssignByIndexId',
			method:'get',
			rownumbers:true,
			autoRowHeight:true,
			pagination:true,
...
<div id="tb" style="padding:5px;height:auto">
	<div style="margin-bottom:5px">
		...
		<a href="javascript:;" onclick="showDisabledProductWin();" class="easyui-linkbutton" iconCls="icon-search">查看全部意向学生</a>
