	...
	<script>
	   ...
	   var jsonStr = '{"stu.stuId":"_stuId","stu.stuNo":"_stuNo",' +
        ...
        function showAddProductWin() {
            ...
            $("#saveWin").window("open"); // 打开指定的window， open表示打开，close表示关闭
        }
        ...
        function saveProduct() {
            if ($("#saveForm").form("validate")) {
            	...
            	var mon =  $("#money").val();
                $.post("<%=path %>/predetermine/save?money="+mon,
                    $("#saveForm").serialize(),
                    function (data) {
        ...
        function showEditProductWin() {
            ...
            var rows = $("#list").datagrid("getSelections");
            var selectRowIndex = $('#list').datagrid('getRowIndex',$('#list').datagrid('getSelected'));//获取选中行索引
            if (rows.length > 1) {
                ...
                if (row) { // 有选中的行
                   ...
                   $("#editForm").form("load", JSON.parse(jsonStr1));
                    $("#editWin").window("open");
                   /*  $("#editMoney").numberbox('setValue',rows[0].setIncome[0].income);
                    $("#editMoneyId").val(rows[0].setIncome[0].incomeId); */
                }
        ...
        function editProduct() {
            if ($("#editForm").form("validate")) {
            	var aa =  $("#money").val();
          		var rows = $("#list").datagrid("getSelections");
                $.post("<%=path %>/predetermine/update?id="+rows[0].stuId+"&money="+aa,
                    $("#editForm").serialize(),
                    function (data) {
                        if (data.result.result == "success") {
        ...
        function showAllProductWin(){
        	$('#list').datagrid({ url: '<%=path %>/predetermine/queryAssign'}); 
        }
        
        function showEnabledProductWin(){
        	$('#list').datagrid({ url: '<%=path %>/predetermine/queryStu'}); 
        }
        
        function showDisabledProductWin(){
        	$('#list').datagrid({ url: '<%=path %>/predetermine/queryStu1'}); 
        }
        
        function qq(value,name){
            $('#list').datagrid({ url: '<%=path %>/predetermine/blurredAllQuery?value1='+value+'&name1='+name});
        }
        ...
        function disable(index){   // 禁用
    		var row = $('#list').datagrid('getData').rows[index];
    		if(row){
    			$.post("<%=path %>/predetermine/roleDisable?id=" + row.stuId,
    				$("#editForm").serialize(),
    				function(data){
    	...
    	function activate(index){  // 启用
    		var row = $('#list').datagrid('getData').rows[index];
    		if(row){
    			$.post("<%=path %>/predetermine/roleActivate?id=" + row.stuId,
    				$("#editForm").serialize(),
    				function(data){
  	...
  	function predetermineAlter(){  // 
  		...
  		if(aa >= 3000){
  		 if (rows) {
               $.post("<%=path %>/predetermine/changePredetermine?id="+rows[0].stuId+"&money="+aa+"&stuStatus="+ss,
            		 function (data) {
                     if (data.result.result == "success") {
...
<table id="list" class="easyui-datagrid"
	   data-options="toolbar:'#tb',
			singleSelect:true,
			collapsible:true,
			url:'<%=path %>/predetermine/queryAssign',
			method:'get',
			rownumbers:true,
			autoRowHeight:true,
			pagination:true,
	...
	<tr>
		...
		<th data-options="field:'stuStatus',width:80" formatter="formatterOpt2">学生状态</th>
		...
		<!-- <th data-options="field:'setIncome',width:60" formatter="stuMoney">金额</th> -->
	</tr>
	</thead>
</table>
...
<div id="tb" style="padding:5px;height:auto">
	<div style="margin-bottom:5px">
		<a href="javascript:;" onclick="showAddProductWin();" class="easyui-linkbutton" iconCls="icon-save">添加</a>
		<a href="javascript:;" onclick="showEditProductWin();" class="easyui-linkbutton" iconCls="icon-edit">编辑</a>
		<a href="javascript:;" onclick="showAlterProductWin();" class="easyui-linkbutton" iconCls="icon-redo">转为正式学生</a>
		<a href="javascript:;" onclick="showAllProductWin();" class="easyui-linkbutton" iconCls="icon-search">查看所有学生</a>
		<a href="javascript:;" onclick="showEnabledProductWin();" class="easyui-linkbutton" iconCls="icon-search">查看所有禁用学生</a>
		<a href="javascript:;" onclick="showDisabledProductWin();" class="easyui-linkbutton" iconCls="icon-search">查看所有启用学生</a>
...
<div id="saveWin" class="easyui-window" title="添加信息" data-options="closed:true" style="width:500px;height:300px;">
	<form id="saveForm" style="margin-left: 100px;margin-right: 100px;">
		<table>
			...
			<tr>
				<td>年龄</td>
				<td><input class="easyui-validatebox easyui-numberbox" name="stu.age" data-options="required:true, max:30"/></td>
			</tr>
			<tr>
				<td>手机号</td>
			...
			<tr>
				<td>学生状态</td>
				<td>
					<select style="width:170px;" class="easyui-combobox" name="stu.stuStatus" data-options="required:true, editable:false"><option value="predetermine">预定学生</option></select>
				</td>
			</tr>
			<tr>
				<td>状态</td>
				<td>
					<select style="width:170px;" class="easyui-combobox" name="stu.status" data-options="required:true, editable:false"><option value="1">启用</option><option value="0">禁用</option></select>
				</td>
			</tr>
			
			<!-- <tr>
				<td>预定学生费用</td>
				<td><input id="money" class="easyui-validatebox easyui-numberbox" name="money" data-options="required:true, min:300"/></td>
...
<div id="editWin" class="easyui-window" title="修改信息" data-options="closed:true" style="width:500px;height:300px;">
	<form id="editForm" style="margin-left: 100px;margin-right: 100px;">
		...
		<table>
			...
			<!-- <tr>
				<td>预定学生费用</td>
				<td><input id="editMoney" class="easyui-validatebox easyui-numberbox" name="editMoney" data-options="required:true, min:300"/>
				<input type="hidden" id="editMoneyId" name="editMoneyId"/></td>
			</tr> -->
			<tr>
				<td>
...
<div id="predetermineWin" class="easyui-window" title="转为预定学生" data-options="closed:true" data-options="iconCls:'icon-save'" style="width:400px;height:300px;">
		<form id = "predetermineForm">
			<table>
			<tr>
				<td>请交转为正式学生的费用</td>
				<td><input id="money" class="easyui-validatebox numberbox" style = "height:30px;" name="money"  data-options="required:true"/></td>
