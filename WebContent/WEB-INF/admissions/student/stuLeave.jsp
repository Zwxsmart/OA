	...
	<script>
        ...
        function showEditProductWin1(index) {
            ...
            if (row) { // 有选中的行
                ...
                var jsonStr1 = jsonStr.replace("_leaveId", row.leaveid).replace("_stuId", row.stuid).replace("_startTime", row.starttime).replace("_endTime", row.endtime).replace("_leaveDay", row.leaveday).replace("_des", row.des).replace("_status", row.status).replace("_firlevel", row.firlevel).replace("_secondLevel", row.secondleavel).replace("_pass", row.pass);
                alert(jsonStr1);
                $("#editForm").form("load", JSON.parse(jsonStr1));
                $("#editWin").window("open");
            }
...
<table id="list" class="easyui-datagrid"
	...
	<tr>
		...
		<th data-options="field:'firlevel',width:100">任课老师审核</th>
		<th data-options="field:'secondLevel',width:100">班主任审核</th>
		<th data-options="field:'pass',width:100">通过状态</th>
...
<div id="tb" style="padding:5px;height:auto">
	<div style="margin-bottom:5px">
		...
		<a href="javascript:;" onclick="showFreezeAllWin();" class="easyui-linkbutton" iconCls="icon-search">查看冻结记录</a>
		<!-- 组合搜索框input -->
...
<!-- 组合搜索框div -->
	<div id="mm" style="width:120px">
		<div data-options="name:'name'">学生</div>
	</div>
	...
	<form id="addForm">
		<table>
			<tr>
				...
				<input id="stuInputa" class="easyui-textbox" name="stuLeave.stuName"/>
				<input id="stuIda" type="hidden" name="stuLeave.stuId"/></a>
				</td>
			</tr>
			<tr>
				<td>开始时间</td>
	...
	<form id="editForm">
		...
		<table>
			<tr>
				...
				<input id="stuInputa1" class="easyui-textbox" name="stuLeave.stuName"/>
				<input id="stuIda1" type="hidden" name="stuLeave.stuId"/></a>
				</td>
			</tr>
			<tr>
				<td>开始时间</td>
