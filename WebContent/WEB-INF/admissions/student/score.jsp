...
<script>
	function showStuAllWin(){
		  var row = $("#list").datagrid("getSelected");
		 $("#stuAllTable").datagrid({url:'<%=path %>/stu/termIdQueryAll?termId='+row.gradeId + '&who='+'gradeId'});// 当点击所有学生框时再加载学生信息
	...
	function showStuAllWin1(){
		  var row = $("#list").datagrid("getSelected");
		 $("#stuAllTable").datagrid({url:'<%=path %>/stu/queryAll?gradeId='+row.gradeId});// 当点击所有学生框时再加载学生信息
