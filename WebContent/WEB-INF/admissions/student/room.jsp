...
<script>
		...
		function showStuWin() {
		  ...
		  $("#stuTable").datagrid({url:'<%=path %>/stu/termIdQueryAll?termId='+rows[0].roomId + '&who='+'roomId'});// 当点击所有学生框时再加载学生信息
		  $("#stuWin").window("open"); // 打开指定的window， open表示打开，close表示关闭
		}
		//添加学生时显示所有学生
		function showStuAllWin1() {
			$("#stuAllTable1").datagrid("reload");
		  $("#stuAllTable1").datagrid({url:'<%=path %>/stu/queryAll'});// 当点击所有学生框时再加载学生信息
				...
				// stuRows.rows.length 班级已有学生总数, rows.length 选择添加了多少个学生, gradeRow.quantity 班级学生最大数
					...
					if (ids != ""){ //假如这个字符串不是空
								...
								$.messager.confirm("提示", "确认删除吗?", function(r){
									if(r){
								$.get("<%=path %>/room/delStuByIds?stuIds=" + ids +"&roomId="+roomRow.gradeId,//传出行元素ids元素
										function(data) {
											$.messager.alert("提示", data.result.message, "info");
											$("#stuAllWin1").window("close");
											$("#stuTable").datagrid("reload");
...
<script>
		...
		function showEditStuWin() {
			  ...
			  $("#stuEditTable").datagrid({url:'<%=path %>/stu/termIdQueryAll?termId='+rows[0].roomId + '&who='+'roomId'});// 当点击所有学生框时再加载学生信息
...
<!-- 显示宿舍所有学生让宿舍添加学生 -->
<div id="stuAllWin1" class="easyui-window" title="所有学生1" data-options="closed:true" style="width:500px;height:300px;">
	...
	<form id="addForm">
		<table>
			...
			<tr>
				<td>状态</td>
				<td>
					<select style="width:100px;" class="easyui-combobox" name="room.status"><option value="1">启用</option><option value="0">禁用</option></select>
				</td>
			</tr>
			<tr>
				<td>
