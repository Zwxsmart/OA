...
<head>
	...
	<script>
        ...
        function okUse(index){   // 确定
    		...
    		var row = $('#list').datagrid('getData').rows[index];
    		if(row){
    			$.post("<%=path %>/empLeave/okEmpLeave?id=" + row.useId+"&goodsCount="+row.goods.quantity+"&useCount="+row.quantity+"&goodsId="+row.goods.goodsId,
    				$("#editForm").serialize(),
    				function(data){
    					$('#list').datagrid('reload');
    			},"json");
    	...
    	function aavate(index){  // 未通过
    		var row = $('#list').datagrid('getData').rows[index];
    		if(row){
    			$.post("<%=path %>/empLeave/empLeaveAct?id=" + row.useId,
    				$("#editForm").serialize(),
    				function(data){
    					$('#list').datagrid('reload'); // 重新加载数据表
    			},"json");
...
<table id="list" class="easyui-datagrid"
	...
	<tr>
		...
		<th data-options="field:'firstLevel',width:100" formatter="formatterOpt3">行政部审核</th>
		<th data-options="field:'dddd',width:90" formatter="formatterOptt1">审核操作</th>
